...
/*   Updated: 2021/06/12 09:39:49 by amoussai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	try 
	{
		...
		Bureaucrat b1("Samuel", 5);
		Form *f1 = new ShrubberyCreationForm("the target");
		std::cout << *f1;
		b1.signForm(*f1);
		f1->execute(b1);
		std::cout << *f1;
	...
	try 
	{
		...
		Form *f2 = new RobotomyRequestForm("the target");
		std::cout << *f2;
		b2.signForm(*f2);
		f2->execute(b2);
		std::cout << *f2;
	...
	try 
	{
		...
		Form *f3 = new PresidentialPardonForm ("the target");
		std::cout << *f3;
		b3.signForm(*f3);
		f3->execute(b3);
		std::cout << *f3;
